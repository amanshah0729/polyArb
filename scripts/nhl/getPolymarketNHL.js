...
// Create outputs directory structure (go up two levels from scripts/nhl/ to root)
...
/**
 * Get NHL tag ID from sports endpoint
 */
async function getNHLTagId() {
  try {
    const sports = await makeRequest(`${POLYMARKET_BASE_URL}/sports`);
    console.log('Available sports:', sports.map(s => ({ sport: s.sport, tags: s.tags })));
    
    // Find NHL sport
    const nhlSport = sports.find(sport => 
      sport.sport && sport.sport.toLowerCase() === 'nhl'
    );

    if (!nhlSport || !nhlSport.tags) {
      throw new Error('NHL sport not found in sports data');
    }

    // Parse tags - it's a comma-separated string like "1,450,100639"
    let tagIds;
    if (typeof nhlSport.tags === 'string') {
      // Check if it's JSON or comma-separated
      if (nhlSport.tags.startsWith('[') || nhlSport.tags.startsWith('{')) {
        try {
          tagIds = JSON.parse(nhlSport.tags);
        } catch (e) {
          // Not JSON, treat as comma-separated string
          tagIds = nhlSport.tags.split(',').map(t => t.trim());
        }
      } else {
        // Comma-separated string
        tagIds = nhlSport.tags.split(',').map(t => t.trim());
      }
    } else if (Array.isArray(nhlSport.tags)) {
      tagIds = nhlSport.tags;
    } else {
      throw new Error('Unexpected tags format');
    }

    // Use the NHL-specific tag (usually the second one, first is often a general sports tag)
    // Based on API response, NHL has tags "1,450,100639" - 450 is likely the NHL-specific tag
    const tagId = Array.isArray(tagIds) ? tagIds[1] || tagIds[0] : tagIds;

    if (!tagId) {
      throw new Error('Could not extract NHL tag ID');
    }

    console.log(`Found NHL sport: ${nhlSport.sport}, tag IDs: ${nhlSport.tags}, using: ${tagId}`);
    return tagId;
  } catch (error) {
    console.error('Error fetching NHL tag ID:', error.message);
    throw error;
  }
}

/**
 * Fetch all NHL events with pagination
 */
async function fetchNHLEvents(tagId) {
  const allEvents = [];
  let offset = 0;
  const limit = 100;
  let hasMore = true;
...
async function main() {
  try {
    console.log('Fetching NHL tag ID...');
    const nhlTagId = await getNHLTagId();

    console.log('Fetching NHL events...');
    const events = await fetchNHLEvents(nhlTagId);
    ...
    const rawFilename = `polymarket_nhl_raw_${timestampStr}_${timestampMs}.json`;
    ...
    const csvFilename = 'polymarket_nhl.csv';
