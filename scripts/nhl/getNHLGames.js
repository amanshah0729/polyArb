...
const OUTPUTS_DIR = path.join(__dirname, '..', '..', 'outputs', 'nhl');
...
https.get(url, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      // Save raw JSON
      const rawFilename = `nhl_games_raw_${timestampStr}_${timestampMs}.json`;
      ...
      let output = '\n=== NHL Games with De-Vigged Moneyline Odds ===\n\n';
      
      if (games.length === 0) {
        output += 'No NHL games found.\n';
        console.log(output);
        return;
      }
      ...
      const txtFilename = `nhl_games_${timestampStr}_${timestampMs}.txt`;
      ...
      // Save CSV file
      const csvFilename = `nhl_games_${timestampStr}_${timestampMs}.csv`;
      ...
      if (arbCsvRows.length > 1) {
        const arbCsvFilename = `nhl_arbitrage_${timestampStr}_${timestampMs}.csv`;
