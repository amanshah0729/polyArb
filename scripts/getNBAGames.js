...
const OUTPUTS_DIR = path.join(__dirname, '..', 'outputs', 'nba');
...
https.get(url, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      // Save raw JSON
      const rawFilename = `nba_games_raw_${timestampStr}_${timestampMs}.json`;
      ...
      let output = '\n=== NBA Games with De-Vigged Moneyline Odds ===\n\n';
      
      if (games.length === 0) {
        output += 'No NBA games found.\n';
        console.log(output);
        return;
      }
      ...
      const txtFilename = `nba_games_${timestampStr}_${timestampMs}.txt`;
      ...
      // Save CSV file
      const csvFilename = `nba_games_${timestampStr}_${timestampMs}.csv`;
      ...
      if (arbCsvRows.length > 1) {
        const arbCsvFilename = `nba_arbitrage_${timestampStr}_${timestampMs}.csv`;
