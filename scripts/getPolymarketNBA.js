...
/**
 * Get NBA tag ID from sports endpoint
 */
async function getNBATagId() {
  try {
    const sports = await makeRequest(`${POLYMARKET_BASE_URL}/sports`);
    
    // Find NBA sport
    const nbaSport = sports.find(sport => 
      sport.sport && sport.sport.toLowerCase() === 'nba'
    );

    if (!nbaSport || !nbaSport.tags) {
      throw new Error('NBA sport not found in sports data');
    }

    // Parse tags - it's a comma-separated string like "1,745,100639"
    let tagIds;
    if (typeof nbaSport.tags === 'string') {
      // Check if it's JSON or comma-separated
      if (nbaSport.tags.startsWith('[') || nbaSport.tags.startsWith('{')) {
        try {
          tagIds = JSON.parse(nbaSport.tags);
        } catch (e) {
          // Not JSON, treat as comma-separated string
          tagIds = nbaSport.tags.split(',').map(t => t.trim());
        }
      } else {
        // Comma-separated string
        tagIds = nbaSport.tags.split(',').map(t => t.trim());
      }
    } else if (Array.isArray(nbaSport.tags)) {
      tagIds = nbaSport.tags;
    } else {
      throw new Error('Unexpected tags format');
    }

    // Use the NBA-specific tag (usually the second one, first is often a general sports tag)
    // Based on API response, NBA has tags "1,745,100639" - 745 is likely the NBA-specific tag
    const tagId = Array.isArray(tagIds) ? tagIds[1] || tagIds[0] : tagIds;

    if (!tagId) {
      throw new Error('Could not extract NBA tag ID');
    }

    console.log(`Found NBA sport: ${nbaSport.sport}, tag IDs: ${nbaSport.tags}, using: ${tagId}`);
    return tagId;
  } catch (error) {
    console.error('Error fetching NBA tag ID:', error.message);
    throw error;
  }
}

/**
 * Fetch all NBA events with pagination
 */
async function fetchNBAEvents(tagId) {
  const allEvents = [];
  let offset = 0;
  const limit = 100;
  let hasMore = true;
...
async function main() {
  try {
    console.log('Fetching NBA tag ID...');
    const nbaTagId = await getNBATagId();

    console.log('Fetching NBA events...');
    const events = await fetchNBAEvents(nbaTagId);
    ...
    const rawFilename = `polymarket_nba_raw_${timestampStr}_${timestampMs}.json`;
    ...
    const csvFilename = 'polymarket_nba.csv';
